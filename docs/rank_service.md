...
## 1. 功能概览
Rank Service 提供高性能排行榜存取与订阅能力，接收“已计分”的榜单更新事件并写入 Redis ZSET/Hash，支持 HTTP 分页查询与 WebSocket 增量推送，面向 10w 参赛规模与 1w QPS 查询场景进行优化。服务本身不负责赛制计算与判题规则，仅作为榜单存储与查询层，便于水平扩展与缓存优化。

## 2. 关键接口与数据结构
- HTTP：`GET /api/v1/contests/:id/leaderboard?page=&page_size=&mode=`
- WS：`GET /api/v1/contests/:id/leaderboard/ws?page=&page_size=&mode=`
- Redis：
...
3) WS 订阅先返回整页 snapshot，后续通过 Redis Pub/Sub 触发 refresh，并对订阅页进行去抖合并刷新。
