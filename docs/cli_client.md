...
- 主要命令：
  ...
  - `problem create|latest|statement|statement-version|statement-update|delete|upload-prepare|upload-sign|upload-complete|upload-abort|publish`
...
- 判题状态相关返回：
  - `submit status` 默认仅返回摘要字段（不含 compile/tests）
  - `submit status include=details` 才返回 compile/tests
  - `submit status` / `submit batch-status` / `judge status` 的 `data.compile.Log` 为编译日志文本（最大 64KB，超出截断）
  - `submit status` / `submit batch-status` / `judge status` 的 `data.tests[].RuntimeLog` 为运行日志文本（最大 64KB，超出截断）
  - `submit status` / `submit batch-status` / `judge status` 的 `data.tests[].CheckerLog` 为 Checker 日志文本（最大 64KB，超出截断）
- 数据包上传相关接口路径：
...
# 获取提交状态摘要
submit status id=SUBMISSION_ID

# 获取提交状态详情（含 compile/tests）
submit status id=SUBMISSION_ID include=details
