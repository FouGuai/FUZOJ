...
## 功能概览
该模块位于 `internal/user/repository`，负责用户与会话相关的数据访问层实现。它包含基于 MySQL 的用户与 Token 仓储，封禁缓存与 Token 黑名单，并在 `UserRepository` 内部通过 Cache-Aside 模式集成用户缓存。用户仓储支持创建用户、按 ID/用户名/邮箱查询、检测用户名/邮箱存在性、更新密码与状态；Token 仓储支持创建、按 hash 查询、按 hash 撤销并加入黑名单、按用户批量撤销。用户缓存支持空值 TTL，并对缓存 TTL 添加随机抖动，降低缓存击穿与雪崩；封禁与黑名单以集合存储并根据过期时间自动延长 TTL，确保状态一致与成本可控。
