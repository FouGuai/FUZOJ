...
func (s *JudgeProcessor) acquireSlot(ctx context.Context, submissionID string) error {
	select {
	case s.sem <- struct{}{}:
		return nil
	case <-ctx.Done():
...
func (s *JudgeProcessor) releaseSlot() {
	select {
	case <-s.sem:
	default:
	}
...
func (s *JudgeProcessor) tryAcquireSlot() bool {
	select {
	case s.sem <- struct{}{}:
		return true
	default:
...
func (s *JudgeProcessor) requeueForPoolFull(ctx context.Context, msg *mq.Message) error {
