...
func TestMergeLimits(t *testing.T) {
	defaults := pmodel.ResourceLimit{CPUTimeMs: 1000, WallTimeMs: 2000, MemoryMB: 256, StackMB: 64, OutputMB: 32, PIDs: 16}
	override := &pmodel.ResourceLimit{CPUTimeMs: 5000, MemoryMB: 512}
	merged := pmodel.MergeLimits(override, defaults)
...
func TestLoadManifest(t *testing.T) {
	...
	m, err := pmodel.LoadManifest(path)
	if err != nil {
