...
- 日志：
  - 使用结构化日志 （gozero 的日志模块）
  - 生产代码必须使用 `logx`，禁止使用代码库自定义 `logger`
  - 生产代码路径中禁止使用 `fmt.Println`

- **所有注释和错误信息必须使用英文**
- 业务错误必须使用 `pkg/errors/error.go` 与 `pkg/errors/code.go` 中定义的统一错误码
- 在关键地方和重要接口处写上清楚的注释
- **除非我明确指定，否则运行时配置信息不能从环境变量中获取，优先从配置文件和配置中心中获取**
...
- 每完成一个模块都需要写一套完整测试
- 测试代码统一放在每个服务的 `tests/` 目录下，包含测试套件与辅助工具
- 优先使用**表驱动测试**
- 命名规范：
  - 文件以 `_test.go` 结尾
  - 测试函数命名为 `TestXxx`
...
- 状态字段（如 `status`）必须使用数字类型（`TINYINT`/`SMALLINT`/`INT`），禁止使用字符串类型（`VARCHAR`/`TEXT`/`ENUM`）存储状态值
- 状态值请在代码中通过常量或枚举映射维护可读语义，数据库内仅保存数字状态码

## 接口设计
接口应该简洁、幂等，只实现一种语义
- 对于服务内部的接口请使用 gRPC 调用
- 前端调用使用 HTTPS
- gRPC 与 HTTP 接口均必须使用 go-zero 规范与实现方式
- **使用 Kafka 时必须做幂等检查（生产者与消费者均需考虑）**
...
服务都要是无状态的
- 端口策略：`gateway` 必须使用固定端口（当前约定为 `8080`）；除 `gateway` 外其他服务默认使用随机端口（`Port: 0` 或 `ListenOn: 0.0.0.0:0`）
