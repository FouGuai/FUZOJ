#ifndef FUZOJ_SRC_JUDGE_EXECUTOR_H_
#define FUZOJ_SRC_JUDGE_EXECUTOR_H_
...
class JudgerExecutor {
 public:
  JudgerExecutor(std::shared_ptr<Runner> runner, std::shared_ptr<Grader> grader)
      : runner_(std::move(runner)), grader_(std::move(grader)) {}

  UNCOPYABLE(JudgerExecutor);
  JudgerExecutor(JudgerExecutor &&judger);
  JudgerExecutor &operator=(JudgerExecutor &&judger);

  std::shared_ptr<Result> Judge();
  ~JudgerExecutor() = default;
